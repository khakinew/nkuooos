...
{     
     /*LAB3 EXERCISE 4: 2211877*/ 
     // 初始化pra_list_head1为空链表
...
_clock_map_swappable(struct mm_struct *mm, uintptr_t addr, struct Page *page, int swap_in)
{
    ...
    /*LAB3 EXERCISE 4: 2211877*/ 
    // link the most recent arrival page at the back of the pra_list_head1 qeueue.
    // 将页面page插入到页面链表pra_list_head1的末尾
    // 将页面的visited标志置为1，表示该页面已被访问
    list_entry_t *head1 = (list_entry_t*) mm->sm_priv;
    list_add_before(head1, entry);  // 将页面插入队列尾部
...
_clock_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
    ...
    while (1) {
        /*LAB3 EXERCISE 4: 2211877*/ 
        // 编写代码
        // 遍历页面链表pra_list_head1，查找最早未被访问的页面
        // 获取当前页面对应的Page结构指针
        // 如果当前页面未被访问，则将该页面从页面链表中删除，并将该页面指针赋值给ptr_page作为换出页面
        // 如果当前页面已被访问，则将visited标志置为0，表示该页面已被重新访问
        if (curr_ptr == head) {
            curr_ptr = list_next(curr_ptr); // 跳过头节点
        }
        ...
        if (!currpage->visited) {
            ...
            list_del(list_prev(curr_ptr)); // 从链表中删除页面
            *ptr_page = currpage;
            break; // 找到页面后退出循环
        }
        else{
            currpage->visited = 0; // 清除访问标志
            curr_ptr = list_next(curr_ptr); // 移至下一个节点
        }
        // 如果已经遍历一圈，退出循环，返回 -1
       /* if (curr_ptr == head) {
            break; // 所有页面都已被访问，退出循环
        }*/
    }

    // 如果遍历结束没有找到可以交换的页面，返回 -1
    if (*ptr_page == NULL) {
        return -1;
    }
